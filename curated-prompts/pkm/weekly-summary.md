...
3. **Read Clockify timesheet**
   - Look for `~/Desktop/Clockify_Time_Report_Detailed_*_<week-start>-<week-end>.csv`
   - Date format in filename is DD_MM_YYYY (e.g., `25_01_2026-31_01_2026`)
   - If not found, ask the user for the file path
   - Parse the CSV to extract:
     - Total hours per project (group by Project column)
     - Total hours per day (group by Start Date)
     - Billable vs non-billable hours
     - Total weekly hours
   - This is the source of truth for actual time spent -- daily notes estimate, Clockify measures

4. **Read ALL project status**
...
4. **Gather GitHub metrics**
   - For each project with a `repo:` property, call:
     ...
     gh api repos/<owner/repo>/commits --jq '[.[] | select(.commit.author.date > "<week-start>")] | length'
     ```
   - Calculate net change (opened - closed)
   - Commits = gauge of attention even when no tickets exist
...
*Source: Clockify_Time_Report_Detailed_DD_MM_YYYY-DD_MM_YYYY.csv*

**Total: XX.Xh** (billable: XX.Xh, non-billable: XX.Xh)

| Project | Hours | Billable | Notes |
|---------|-------|----------|-------|
| Chiro | X.Xh | Yes | [key activities] |
| Resin | X.Xh | Yes | [key activities] |
| Coursework | X.Xh | No | [what course] |
...
| Project | Open | Closed | Opened | Net | Commits |
|---------|------|--------|--------|-----|---------|
