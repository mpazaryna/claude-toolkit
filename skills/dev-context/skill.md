...
**Process**:
1. Create file `docs/specs/{num}-{short-name}.md`
2. Generate spec with issue link, status, and implementation approach
3. Return file path
...
**Note**: Most decisions don't need formal ADRs. They get captured in devlogs and extracted to ADRs during periodic maintenance. Use ADRs sparingly for decisions that truly warrant formal documentation.
...
| Request | Mode | Output |
|---------|------|--------|
| "Set up context for issue #115" | Issue | `docs/specs/115-feature.md` |
| "Create working space for auth" | Issue | `docs/specs/{num}-auth.md` |
...
2. **Update** the spec file as you work
3. **Extract** patterns to `docs/adr/` if reusable
4. **Delete** spec file when issue closes

### ADRs
1. **Create** for major decisions
2. **Never edit** once accepted (write new ADR if decision changes)
3. **Reference** from `CLAUDE.md` for discoverability
...
| `dev-explore` | Analyze codebases, export docs for LLM tools |
...
- `docs/adr/ADR-003-documentation-maintenance-workflows.md` - When to extract patterns
